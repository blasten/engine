...
void PatchedRTree::insert(const SkRect boundsArray[], int N) {
  SkASSERT(0 == fCount);
  ...
  for (int i = 0; i < N; i++) {
      ...
      FML_DLOG(ERROR) << "bounds: " << bounds.fLeft << "x" << bounds.fTop <<  "x" << bounds.fRight <<  "x" << bounds.fBottom;
...
PatchedRTree::Node* PatchedRTree::allocateNodeAtLevel(uint16_t level) {
...
int PatchedRTree::CountNodes(int branches) {
  if (branches == 1) {
      return 1;
  }
...
PatchedRTree::Branch PatchedRTree::bulkLoad(std::vector<Branch>* branches, int level) {
...
void PatchedRTree::search(const SkRect& query, std::vector<int>* results) const {
  if (fCount > 0 && SkRect::Intersects(fRoot.fBounds, query)) {
      this->search(fRoot.fSubtree, query, results);
  }
}

void PatchedRTree::search(Node* node, const SkRect& query, std::vector<int>* results) const {
...
size_t PatchedRTree::bytesUsed() const {
  size_t byteCount = sizeof(PatchedRTree);
  byteCount += fNodes.capacity() * sizeof(Node);
  return byteCount;
}

sk_sp<SkBBoxHierarchy> PatchedRTreeFactory::operator()() const {
  return sk_make_sp<PatchedRTree>();
}
